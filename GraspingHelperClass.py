...
# 0.05 both
    # TODO
    if workspace:
        y_offset = 0.017 + 0.016 + in_to_m(0.5)
        x_offset = 0.025 + 0.004 - 0.005 - in_to_m(0.075) - in_to_m(0.3) - 0.017  + in_to_m(1.21) - in_to_m(2) # 0m is the camera offset from the gripper center

        x_offset = 0.005
        y_offset = 0.017 + 0.01# For 2D camera
    else:
        y_offset = 0.017 + 0.016 - in_to_m(2)
        x_offset = 0.025 - 0.001 - in_to_m(0.075) - in_to_m(0.15) - in_to_m(0.15) - 0.018 + in_to_m(1.28) - in_to_m(0.75)# 0m is the camera offset from the gripper center

        y_offset = 0.017 + 0.005 
        x_offset = 0.025 - 0.025
...
def graspExecute(limb, gripper, W, H, Ang, x_ref, y_ref, table, group, workspace):
    ...
    # TODO
    if workspace:
        print("Going to the LEFT table")
        y_offset = 0.017 + 0.016 - in_to_m(2.75)
        y_offset = 0.017 + 0.016 # For 2D camera
        x_offset = 0.025 + 0.004 - 0.005 - in_to_m(0.075) - in_to_m(0.3) - 0.017 - in_to_m(0.5) + 0.05 # 0m is the camera offset from the gripper center


        x_offset = 0.005
        y_offset = 0.017 + 0.03 - in_to_m(0.5)# For 2D camera
    else:
        print("Going to the RIGHT table")
        y_offset = 0.017 + 0.016 - in_to_m(2.75) + in_to_m(3)
...
def take_picture(camera_port, ramp_frames):
    ...
    # print("Using camera port " + camera_port)
...
def detect_objects(fruit_number, img):
    ...
    lower_range_orange = np.array([0, 100, 100], dtype=np.uint8)
...
def dropBlockByImageExecute(limb, gripper, W, H, Ang, x_ref, y_ref, table, group, workspace):
    ...
    x_offset = 0.025 + 0.004 - 0.005 - in_to_m(0.075) - in_to_m(0.3) - 0.017 + in_to_m(0.125)# 0m is the camera offset from the gripper center
    # else:
    #     y_offset = 0 + 0.015 + 0.018
    #     x_offset = in_to_m(1.8) - 0.025
    # y_offset = 0.005
