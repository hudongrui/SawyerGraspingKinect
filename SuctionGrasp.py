...
while iterations != 0:
    ...
    for drop_off_location in task:
        ...
        while square_list is None:
            if timeout > 20:
                rospy.logerr("No block exists in the frame.")
                exit()
            img = Gp.take_picture(camera_port, 30)
    ...
    #    Gp.move_move(limb, group, positions=moveJoint)
        ...
        ##########
        # Establishing Serial Connection for Suction Gripper
        #########
        # ser = None;


        Gp.graspExecuteSuction(limb, gripper, W, H, Ang, worldVec[0], worldVec[1], 1, group, temp_workspace, ser)
        # Gp.move_move(limb, group, moveJoint)
        # Gp.graspExecuteSuction
        # ser.write("open");

        Gp.dropExecuteSuction(limb,gripper,drop_off_location, dQ, group, operation_height, temp_workspace, ser)
